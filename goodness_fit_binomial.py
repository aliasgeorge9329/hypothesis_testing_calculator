...
from color import *
table = PrettyTable(['X', 'Observed Frequency', 'Binomial Probabilities', 'Expected Frequency'])
reduced_table = PrettyTable(['Observed Frequency', 'Binomial Probabilities', 'Expected Frequency'])


def goodness_fit_binomial():
    # Binomial
    alpha = float(input("Level of significance: "))
    n_ = float(input("Enter the n: "))
    ...
    print(f"\n{bcolors.BOLD}{bcolors.WARNING}Null hypothesis: Random variable has a Binomial distribution with n = {n_} and p = {p_}.")
    print(f"Alternative hypothesis: Random variable does not have the Binomial distribution with n = {n_} and p = {p_}.{bcolors.ENDC}\n")
    ...
    if total_chi_square > critical_value:
        print(
            f"Since χ^2 = {total_chi_square} exceed {critical_value}, the null 'Data fit to the distribution'"
        ...
        print(f"Since χ^2 = {total_chi_square} does not exceed {critical_value}, the null hypothesis cannot be "
              f"rejected; we cannot reject that the Binomial distribution with n = {n_} and p = {p_} provides a good fit at level "
              f"⍺ = {alpha}.")
