...
from slicing import *
from color import *
table = PrettyTable(['X', 'Observed Frequency', 'Normal Probabilities of Interval area'])
reduced_table = PrettyTable(['Observed Frequency', 'Normal Probabilities of Interval area', 'Expected Frequency'])


def goodness_fit_normal():
    # Normal
    ...
    if selection == 1:
        ...
        a["probability"] = round(norm.cdf(a["x_h"], loc=mean, scale=std_devi),4)
        print("\n")
        data.append(a)
        ...
        for i in range(1, no-1):
            ...
            a["probability"] = round((norm.cdf(a["x_h"], loc=mean, scale=std_devi) - norm.cdf(a["x_l"], loc=mean, scale=std_devi)), 4)
            print("\n")
            last_final = a["x_h"]
            data.append(a)
        ...
        a["probability"] = round((1 - norm.cdf(a["x_l"], loc=mean, scale=std_devi)), 4)
        print("\n")
        data.append(a)
    ...
    print(f"\n{bcolors.BOLD}{bcolors.WARNING}Null hypothesis: Random variable has a Normal distribution with μ = {mean} "
          f"and standard deviation = {std_devi}.")
    print(f"Alternative hypothesis: Random variable does not have the Normal distribution with μ = {mean} "
    ...
    if total_chi_square > critical_value:
        ...
        print(f"Since χ^2 = {total_chi_square} does not exceed {critical_value}, the null hypothesis cannot be "
              f"rejected; we cannot reject that the Normal distribution with μ = {mean} and standard deviation "
