...
def goodness_fit_geometric():
    ...
    print(f"\n{bcolors.BOLD}{bcolors.WARNING}Null hypothesis: Random variable has a Geometric distribution with p= {p_}.")
    print(f"Alternative hypothesis: Random variable does not have the Geometric distribution with p= {p_}.{bcolors.ENDC}\n")
    ...
    if total_chi_square > critical_value:
        ...
        print(f"Since χ^2 = {total_chi_square} does not exceed {critical_value}, the null hypothesis cannot be "
              f"rejected; we cannot reject that the Geometric distribution with p = {p_} provides a good fit at level "
              f"⍺ = {alpha}.")
