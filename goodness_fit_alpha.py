from scipy.stats import alpha, chi2
from prettytable import PrettyTable
from slicing import *
from color import *
table = PrettyTable(['X', 'Observed Frequency', 'Alpha Probabilities of Interval area'])
reduced_table = PrettyTable(['Observed Frequency', 'Alpha Probabilities of Interval area', 'Expected Frequency'])


def goodness_fit_alpha():
    # Alpha
    ...
    if selection == 1:
        ...
        a["probability"] = round(alpha.cdf(a["x_h"], a=a_, loc=mean, scale=std_devi),4)
        print("\n")
        data.append(a)
        ...
        for i in range(1, no-1):
            ...
            a["probability"] = round((alpha.cdf(a["x_h"], a=a_,  loc=mean, scale=std_devi) - alpha.cdf(a["x_l"], a=a_, loc=mean, scale=std_devi)), 4)
            print("\n")
            last_final = a["x_h"]
            data.append(a)
        ...
        a["probability"] = round((1 - alpha.cdf(a["x_l"], a=a_,  loc=mean, scale=std_devi)), 4)
        print("\n")
        data.append(a)
    ...
    print(f"\n{bcolors.BOLD}{bcolors.WARNING}Null hypothesis: Random variable has a Alpha distribution with a= {a_}, μ = {mean} and standard deviation = {std_devi }.")
    print(f"Alternative hypothesis: Random variable does not have the Alpha distribution with a= {a_} μ = {mean}, and standard deviation = {std_devi }.{bcolors.ENDC}\n")
    ...
    if total_chi_square > critical_value:
        ...
        print(f"Since χ^2 = {total_chi_square} does not exceed {critical_value}, the null hypothesis cannot be "
              f"rejected; we cannot reject that the Alpha distribution with a= {a_} μ = {mean} and standard deviation = {std_devi } provides a good fit at level "
              f"⍺ = {alpha_}.")
