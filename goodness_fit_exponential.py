...
from color import *
import math
table = PrettyTable(['X', 'Observed Frequency', 'Exponential Probabilities of Interval area'])
reduced_table = PrettyTable(['Observed Frequency', 'Exponential Probabilities of Interval area', 'Expected Frequency'])
...
def goodness_fit_exponential():
    # Exponential
    alpha_ = float(input("Level of significance: "))
    lam_ = float(input("Enter the parameter λ: "))
    ...
    if selection == 1:
        ...
        a["probability"] = round(exponential(a["x_h"], lam_), 4)
        print("\n")
        data.append(a)
        ...
        for i in range(1, no-1):
            ...
            a["probability"] = round((exponential(a["x_h"], lam_) - exponential(a["x_l"], lam_)), 4)
            print("\n")
            last_final = a["x_h"]
            data.append(a)
        ...
        a["probability"] = round((1 - exponential(a["x_l"], lam_)), 4)
        print("\n")
        data.append(a)
    ...
    print(f"\n{bcolors.BOLD}{bcolors.WARNING}Null hypothesis: Random variable has a Exponential distribution with λ = {lam_}.")
    print(f"Alternative hypothesis: Random variable does not have the Exponential distribution with λ = {lam_}.{bcolors.ENDC}\n")
    ...
    if total_chi_square > critical_value:
        ...
        print(f"Since χ^2 = {total_chi_square} does not exceed {critical_value}, the null hypothesis cannot be "
              f"rejected; we cannot reject that the Exponential distribution with λ = {lam_} provides a good fit at level "
              f"⍺ = {alpha_}.")
